...
class Zhihu:
	...
	def get_answer(self, url):
		self.dr.get(url)
		article = {}
		article['question'] = self.dr.find_element_by_css_selector(".question_link").text
		article['author'] = self.dr.find_element_by_css_selector(".author-link-line").text
		article['answer'] = self.dr.find_element_by_css_selector(".zh-summary summary clearfix").get_attribute("innerHTML")
...
if __name == '__main__':
	with Zhihu() as zhihu:
		...
		report = ZhihuReporter(report_name)
